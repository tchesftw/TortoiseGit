...
void CCommitDlg::OnOK()
{
	...
	CMassiveGitTask mgtReAddAfterCommit(L"add --ignore-errors -f"); // these two commands will not be executed if staging support is enabled
	...
	// If staging support is enabled, simply assume the index is up-to-date and skip all the code dealing with it
			...
			if(p && !(p->m_Action&CTGitPath::LOGACTIONS_UNVER) )
			{
				if (m_bStagingSupport)
				{
					...
					if (!p->GetGitOldPathString().IsEmpty())
						cmd.Format(L"git.exe diff%s -- \"%s\" \"%s\"", m_bPartialStagingTextCurrentlyIsShow ? L" --cached" : L"",
							static_cast<LPCTSTR>(p->GetGitOldPathString()), static_cast<LPCTSTR>(p->GetGitPathString()));
					else
						cmd.Format(L"git.exe diff%s -- \"%s\"", m_bPartialStagingTextCurrentlyIsShow ? L" --cached" : L"",
							static_cast<LPCTSTR>(p->GetGitPathString()));
				}
				else
				{
					CString head = L"HEAD";
					if (m_bCommitAmend == TRUE && m_bAmendDiffToLastCommit == FALSE)
						head = L"HEAD~1";
					if (!p->GetGitOldPathString().IsEmpty())
						cmd.Format(L"git.exe diff %s -- \"%s\" \"%s\"", static_cast<LPCTSTR>(head),
								   static_cast<LPCTSTR>(p->GetGitOldPathString()), static_cast<LPCTSTR>(p->GetGitPathString()));
					else
